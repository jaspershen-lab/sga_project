...
sxtTools::setwd_project()
setwd("data/metabolomics/sga_biomarker_feature")

####load data
load("../data_preparation/peak/expression_data")
load("../data_preparation/peak/sample_info")
load("../data_preparation/peak/variable_info")
...
for(i in 1:length(sample_idx)){
  ...
  discovery_data = discovery_data[,marker_rf]
  validation_data = validation_data[,marker_rf]
...
# sample_idx =
# seq(1,length(idx), by = 28) %>% lapply(function(x){idx[x:(x+27)]})
# sample_idx = 
#   sample_idx %>% 
#   lapply(function(x){
#     x[!is.na(x)]
